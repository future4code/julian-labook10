...
export const feedPostsEndpoint = async (req: Request, res: Response): Promise<void> => {
    try {   
        const token = req.headers.authorization as string
        ...
        const postDb = new PostsDatabase()
        const feedPosts = await postDb.feedPosts(authentication.id)
...
export const feedFilterByTypeEndpoint = async (req: Request, res: Response): Promise<void> =>{
    try {
        ...
        console.log(type)
        const postDb = new PostsDatabase()
        const feedFilterData = await postDb.feedFilter(authentication.id, type)
